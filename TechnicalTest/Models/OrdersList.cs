...
public partial class project1 {
    ...
    /// ordersList
    /// </summary>
    public static OrdersList ordersList
    {
        get => HttpData.Get<OrdersList>("ordersList")!;
        set => HttpData["ordersList"] = value;
    }

    /// <summary>
    /// Page class for Orders
    /// </summary>
    public class OrdersList : OrdersListBase
    {
        // Constructor
        public OrdersList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public OrdersList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class OrdersListBase : Orders
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "ordersList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fOrderslist";
        ...
        public OrdersListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "OrdersAdd?" + Config.TableShowDetail + "=";
            ...
            MultiDeleteUrl = "OrdersDelete";
            MultiUpdateUrl = "OrdersUpdate";
        ...
        // Page name
        public string PageName => "OrdersList";
        ...
        // Constructor
        public OrdersListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected bool RestoreFilterList() {
            ...
            if (filter?.TryGetValue("x_SalesOrder", out sv) ?? false) {
                ...
                SalesOrder.AdvancedSearch.SearchOperator = filter["z_SalesOrder"];
                SalesOrder.AdvancedSearch.SearchCondition = filter["v_SalesOrder"];
                SalesOrder.AdvancedSearch.SearchValue2 = filter["y_SalesOrder"];
                SalesOrder.AdvancedSearch.SearchOperator2 = filter["w_SalesOrder"];
                SalesOrder.AdvancedSearch.Save();
            }

            // Field OrderDate
            if (filter?.TryGetValue("x_OrderDate", out sv) ?? false) {
                OrderDate.AdvancedSearch.SearchValue = sv;
                OrderDate.AdvancedSearch.SearchOperator = filter["z_OrderDate"];
                OrderDate.AdvancedSearch.SearchCondition = filter["v_OrderDate"];
        ...
        public async Task RenderListOptions()
        {
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fOrderslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fOrderslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            // "detail_Items"
            listOption = ListOptions["detail_Items"];
            isVisible = true;
            if (isVisible) {
                string caption, title, url;
                var body = Language.Phrase("DetailLink") + Language.TablePhrase("Items", "TblCaption");
                ...
                if (detailPage?.DetailEdit ?? false) {
                    ...
                    url = GetEditUrl(Config.TableShowDetail + "=Items");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""Orders"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            items = Resolve("Items")!;
            if (items != null) {
                caption = Language.Phrase("Add") + "&nbsp;" + Caption + "/" + items.Caption;
                title = Language.Phrase("Add", true) + "&nbsp;" + Caption + "/" + items.Caption;
                item.Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" + title + "\" data-caption=\"" + title + "\" href=\"" + HtmlEncode(AppPath(GetAddUrl(Config.TableShowDetail + "=Items"))) + "\">" + caption + "</a>";
                item.Visible = (items.DetailAdd);
                if (item.Visible) {
                    if (detailTableLink != "")
                        detailTableLink += ",";
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fOrderslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(ordersList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(ordersList.RowCount) + "_Orders");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(ordersList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && ordersList.RowType == RowType.Add || IsEdit && ordersList.RowType == RowType.Edit) // Inline-Add/Edit row
