...
public partial class project1 {
    ...
    public class ItemGridBase : Item
    {
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            // Load master record
            if (CurrentMode != "add" && !Empty(MasterFilterFromSession) && CurrentMasterTable == "Order") {
                order = Resolve("Order")!;
                if (order != null) {
                    using (var rsmaster = await order.LoadReader(DbMasterFilter)) { // Note: Use {}
                        MasterRecordExists = rsmaster != null && await rsmaster.ReadAsync();
                        if (!MasterRecordExists) {
                            FailureMessage = Language.Phrase("NoRecord"); // Set no record found
                            return Terminate("OrderList"); // Return to master page
                        } else {
                            order.LoadListRowValues(rsmaster);
                        }
                    }
                    order.RowType = RowType.Master; // Master row
                    await order.RenderListRow(); // Note: Do it outside "using" // DN
                }
            }
