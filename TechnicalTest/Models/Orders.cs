...
public partial class project1 {
    ...
    {
        get => HttpData.Resolve<Orders>("Orders");
        set => HttpData["Orders"] = value;
    }

    /// <summary>
    /// Table class for Orders
    /// </summary>
    public class Orders : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<SqlDbType> SalesOrder;

        public readonly DbField<SqlDbType> OrderDate;

        public readonly DbField<SqlDbType> Customer;
        ...
        public Orders()
        {
            ...
            Name = "Orders";
            Type = "TABLE";
            UpdateTable = "dbo.Orders"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            ID = new (this, "x_ID", 3, SqlDbType.Int) {
                ...
                CustomMessage = Language.FieldPhrase("Orders", "ID", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("ID", ID);

            // SalesOrder
            SalesOrder = new (this, "x_SalesOrder", 202, SqlDbType.NVarChar) {
                Name = "SalesOrder",
                Expression = "[SalesOrder]",
                UseBasicSearch = true,
                BasicSearchExpression = "[SalesOrder]",
                DateTimeFormat = -1,
                VirtualExpression = "[SalesOrder]",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
        ...
        public string DetailUrl
        {
            get {
                string detailUrl = "";
                if (CurrentDetailTable == "Items" && items != null) {
                    detailUrl = items.ListUrl + "?" + Config.TableShowMaster + "=" + TableVar;
        ...
        public async Task<int> UpdateAsync(object data, object? where, Dictionary<string, object>? rsold)
        {
            ...
            if (rsold != null) {
                // Cascade Update detail table 'Items'
                cascadeUpdate = false;
                rscascade.Clear();
                ...
                if (cascadeUpdate) {
                    ...
                    items = Resolve("Items")!;
                    if (items != null) {
                        var rows = await items.Connection.GetRowsAsync(items.GetSql("[OrderID] = " + QuotedValue(rsold["ID"], DataType.Number, "DB")));
                        foreach (var rsdtlold in rows) {
                            rskey = items.GetRowFilterAsDictionary(rsdtlold);
                            var rsdtlnew = new Dictionary<string, object>(rsdtlold);
                            foreach (var (key, value) in rscascade)
                                rsdtlnew[key] = value;
                            bool update = items.Invoke("RowUpdating", new object[] { rsdtlold, rsdtlnew }) is bool b && b; // Call Row Updating event
                            if (update)
                                update = await items.UpdateAsync(rscascade, rskey, rsdtlold) >= 0; // Update
                            if (!update)
                                return -1;
                            items.Invoke("RowUpdated", new object[] { rsdtlold, rsdtlnew }); // Call Row Updated event
                        }
                    }
                }
            }
        ...
        public async Task<int> DeleteAsync(object? data, object? where = null)
        {
            ...
            if (row != null) {
                // Cascade delete detail table 'Items'
                items = Resolve("Items")!;
                if (items != null) {
                    dtlrows = await Connection.GetRowsAsync(items.GetSql("[OrderID] = " + QuotedValue(row["ID"], DataType.Number, "DB")));
                    delete = dtlrows.All(dtlrow => items.Invoke("RowDeleting", new object[] { dtlrow }) is bool b && b); // Call Row Deleting event
                    if (delete) {
                        foreach (var dtlrow in dtlrows) {
                            delete = ConvertToBool(await items.DeleteAsync(dtlrow)); // Delete
                            if (!delete)
                                break;
                        }
                    }
                    if (delete)
                        dtlrows.ForEach(dtlrow => items.Invoke("RowDeleted", new object[] { dtlrow })); // Call Row Deleted event
                }
            }
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                SalesOrder.DbValue = row["SalesOrder"]; // Set DB value only
                OrderDate.DbValue = row["OrderDate"]; // Set DB value only
                Customer.DbValue = row["Customer"]; // Set DB value only
        ...
        {
            if (SameString(pageName, "OrdersView"))
                return Language.Phrase("View");
            else if (SameString(pageName, "OrdersEdit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "OrdersAdd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "OrdersView",
                Config.ApiAddAction => "OrdersAdd",
                Config.ApiEditAction => "OrdersEdit",
                Config.ApiDeleteAction => "OrdersDelete",
                Config.ApiListAction => "OrdersList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "OrdersList";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("OrdersView", parm);
            else
                url = KeyUrl("OrdersView", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "OrdersAdd";
        ...
        public string GetEditUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("OrdersEdit", parm);
            else
                url = KeyUrl("OrdersEdit", Config.TableShowDetail + "=");
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("OrdersList", "action=edit"))); // DN
        ...
        public string GetCopyUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("OrdersAdd", parm);
            else
                url = KeyUrl("OrdersAdd", Config.TableShowDetail + "=");
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("OrdersList", "action=copy"))); // DN
        ...
        public string GetDeleteUrl(string parm = "")
        {
            return UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
                ...
                : AppPath(KeyUrl("OrdersDelete", parm)); // DN
        }
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            if (dr == null)
                return;
            ID.SetDbValue(dr["ID"]);
            SalesOrder.SetDbValue(dr["SalesOrder"]);
            OrderDate.SetDbValue(dr["OrderDate"]);
            Customer.SetDbValue(dr["Customer"]);
